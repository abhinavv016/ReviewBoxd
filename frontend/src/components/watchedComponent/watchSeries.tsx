...
export default function WatchedSeries({ id, media_type }: WatchedSeriesProps) {
    ...
    return (<>
        <div className="absolute flex left-1/2 translate-x-80 flex-col items-center bg-[#1b263b] w-80 mt-15 rounded-md">
            ...
            <div className="border-t border-slate-100 w-full mt-2"></div>
            <div
                className="flex justify-center items-center h-10 w-full hover:text-[#8FA1B9] cursor-pointer"
        ...
        <AnimatePresence>
            {isVisible && <SeasonsComp onClose={handleCloseClick} media_type={media_type} id={id} />}
        </AnimatePresence>
    </>
    )
}